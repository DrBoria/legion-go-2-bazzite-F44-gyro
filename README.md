...
**What v3.3 adds.** v3.2 already decoded the two IMU triads and shipped complete raw frames;
v3.3 makes the decode **exhaustive** — every field of the 60-byte `XInputDataReport` (inside
the 64-byte hidraw frame) is decoded by name on a `DECODE LEGION-SRC ... XFULL` line whenever
it changes, so a single remote log can tell whether ANY field carries data on that unit:

- both enums — `mode=` (xinput/dinput/fps) and `state=` (attached/detached/connecting);
- both controller battery levels, the sticks (`sticks=(lsx,lsy,rsx,rsy)`) and **all 32
  buttons by name** (`btn=[a,b,x,y,lb,rb,y1,y2,y3,m1,m2,m3,view,menu,...]`);
- analog triggers (`trig=`), mouse wheel (`wheel=`), touch-pad coordinates (`touch=`),
  both low-quality gyros (`lqgyro=`) and both IMU timestamps (`ts=`);
- both full accelerometer + gyroscope triads by name — `accel_l` / `left_gyro` (bytes 35–46)
  and `accel_r` / `right_gyro` (bytes 48–59, printed in the exact `(y,x,z)` order
  InputPlumber reads them in [`hid_report.rs`](src/drivers/lego/hid_report.rs));
- the raw trailer (`tr=`) plus the complete raw frame in hex on every decode line.

This closes the last capture gap from the "log EVERYTHING" requirement: nothing in the Legion
XInput frame is left undecoded.

## What's new in v10

**v10** was the previous release (superseded by [v10.1](#whats-new-in-v101)) — a
**logger-only** upgrade to the **v3.2 diagnostic logger**
(needed to diagnose a "gyro dead in-game" report from ANOTHER machine). The InputPlumber
binary is **unchanged** — still the same `c9a4bfa8` build as v9 (prebuilt sha256:
`c9a4bfa800a2c1bca078c41ddfcb0131351cd8f5402d8a5cdd4963ca13476e00`).

**Why.** On this machine the real in-game rotation test (ESO, 2026-09-03 17:35:30–17:35:44)
proved the full gyro path works in gaming mode: the virtual Steam Deck (`deck-uhid`
`28de:12f0`) carried the real gyro (motion magnitude up to **2852**, e.g.
`gyr=(1704,325,-823)`), fed live from the physical Legion XInput `right_gyro` bytes 54–59
(`x` up to 2051). So the gaming profile is NOT the problem.

**What v3.2 adds** — enough to tell whether ANOTHER unit's gyro source itself is alive, from a
single log:
- full physical Legion XInput IMU decode — `left_gyro` (bytes 41–46) and `right_gyro`
  (bytes 54–59), the exact raw bytes that feed InputPlumber's `MultiGyro`
  (`DECODE LEGION-SRC ... right_gyro=(y,x,z)` lines);
- complete raw 64-byte XInput frames periodically and on motion;
- IIO ↔ LEGION-SRC ↔ DECK correlation (fast IIO sampling + coalesced DECK reads during motion).

**For a remote tester with dead gyro** (the open SamTsuki case): install v10.1 with
`./install.sh --log`, start the same game, rotate the device ~15 s in-game, then send back
`/var/log/ip-gyro-logger.log` — the `DECODE LEGION-SRC ... right_gyro=` lines tell
immediately whether his IMU reaches the controller (see
[Diagnostic logger](#diagnostic-logger-v33-optional)).

## What's new in v9

**v9** was the previous release (superseded by [v10](#whats-new-in-v10)) — the **sleep-in-game fix**. It ships the `c9a4bfa8` build
...
[Diagnostic logger](#diagnostic-logger-v33-optional) section below.
...
`install.sh` now supports **two modes**: a plain `./install.sh` (binary install only) and `./install.sh --log` (`-l` / `--logger` are aliases), which additionally installs and enables the passive [diagnostic logger](#diagnostic-logger-v33-optional). See the install section below.

## Install (prebuilt binary)

Download **`inputplumber-legiongo2-gyro-v10.1.tar.gz`** from the **Releases** page, extract it and run:

```bash
tar xzf inputplumber-legiongo2-gyro-v10.1.tar.gz
./install.sh        # plain install — binary + profile + power fixes + auto gyro-reset unit, restarts inputplumber, ensures the logger is OFF
./install.sh --log  # ONE command: same as above PLUS installs & enables the v3.3 diagnostic logger, which captures EVERYTHING into /var/log/ip-gyro-logger.log
```
...
6. **optionally** the passive diagnostic logger (only with `--log`; plain `./install.sh` makes sure it is fully removed — see [Diagnostic logger](#diagnostic-logger-v33-optional)).
...
## Diagnostic logger (v3.3, optional)

In gaming mode the virtual Steam Deck controller could previously map only A/B (no joysticks, no gyro). To make such issues diagnosable after the fact, this repo ships a **passive diagnostic logger**: pure Python 3 standard library, no dependencies, no rebuild. **v10.1** ships the **v3.3** logger, which captures every source into a single log (`/var/log/ip-gyro-logger.log`) via the one command `./install.sh --log` — the per-app Steam Input activation markers described in [What's new in v9](#2-diagnostic-logger-v31), the physical IMU decode described in [What's new in v10](#whats-new-in-v10), and the full per-field `XFULL` decode described in [What's new in v10.1](#whats-new-in-v101).

Files in [`logger/`](logger/):

- `logger/ip-gyro-logger.py` — records timestamped, greppable evidence: udev device add/remove events, `/proc/bus/input/devices` snapshots, IIO gyro/accel samples, evdev key/axis events, hidraw flow into the virtual deck, Steam registry verdicts, InputPlumber's journal, STATE session markers, and a heartbeat / session (gamescope/desktop) transitions. **v3.1** added: per-app Steam Input activation from Steam's `controller_ui.txt` and a running-game tracker (AppID + loaded config). **v3.2** added: full physical Legion XInput IMU decode (`DECODE LEGION-SRC ... left_gyro=(41–46) right_gyro=(54–59)`), complete raw 64-byte frames on a timer/motion, and IIO ↔ LEGION-SRC ↔ DECK correlation during motion. **v3.3** adds: per-frame decode of **EVERY** field of the Legion XInput report on a `DECODE LEGION-SRC ... XFULL` line — enums, batteries, sticks, all 32 buttons by name, triggers, wheel, touch, low-quality gyros, both IMU timestamps and both full accel+gyro triads by name, plus the raw trailer and full frame hex — so any change in the physical frame is captured.
...
Steam registering **`28de-12f0`** (not `28de-12fb`) during the gaming-mode `deck-uhid` window is the expected v8/v9/v10/v10.1 behaviour.
...
- `inputplumber-legiongo2-gyro-v4.resume-gamefix` — prebuilt modified binary for **v10.1** (unchanged since v9/v10; Release asset: `inputplumber-legiongo2-gyro-v10.1.tar.gz`); sha256 `c9a4bfa800a2c1bca078c41ddfcb0131351cd8f5402d8a5cdd4963ca13476e00`
...
- `logger/` — passive diagnostic logger (v3.3: `ip-gyro-logger.py` + `ip-gyro-logger.service`), installed by `./install.sh --log` (see [Diagnostic logger](#diagnostic-logger-v33-optional))
